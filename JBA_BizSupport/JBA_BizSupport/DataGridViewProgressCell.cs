...
namespace JBA_BizSupport
{
    public class DataGridViewProgressCell : DataGridViewImageCell
    {
            ...
            DataGridViewPaintParts paintParts)
        {
            ...
            float percentage = ((float)progressVal / 70.0f); // Need to convert to float before division; otherwise C# returns int which is 0 for anything but 100%.
            ...
            float textWidth = TextRenderer.MeasureText("□□□□□□□□□□□", cellStyle.Font).Width;
            ...
            if (percentage >= 0.0)
            {
                ...
                g.FillRectangle(new SolidBrush(_ProgressBarColor), cellBounds.X + 2, cellBounds.Y + 2, Convert.ToInt32((percentage * cellBounds.Width * 0.97)), cellBounds.Height / 1 - 5);
                //Draw text
                //g.DrawString(progressVal.ToString() + "%", cellStyle.Font, foreColorBrush, posX, posY);
                switch (progressVal)
                {
                    case 10:
                        g.DrawString("仮払申請受付", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 20:
                        g.DrawString("データ入力中", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 30:
                        g.DrawString("振込依頼ファイル出力済", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 35:
                        g.DrawString("振込確認済", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 40:
                        g.DrawString("受領書発行済", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 45:
                        g.DrawString("受領書回収済", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 50:
                        g.DrawString("仮払仕訳データ出力済", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 60:
                        g.DrawString("精算済", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    case 70:
                        g.DrawString("精算仕訳データ出力済", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                    default:
                        g.DrawString("", cellStyle.Font, foreColorBrush, posX, posY);
                        break;
                }
            }
            else
